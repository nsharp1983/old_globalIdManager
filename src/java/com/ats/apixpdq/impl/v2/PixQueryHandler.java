...
class PixQueryHandler extends BaseHandler implements Application {
    ...
    public Message processMessage(Message msgIn) throws ApplicationException, HL7Exception {
    	...
    	try {
	        ...
	        {
	           throw new ApplicationException( "PIX HL7查询消息 失败，有效的查询消息为：QBP^Q23.");
	        } 
		}
    ...
    throws PixPdqException, HL7Exception {
        ...
        if (!requestDomainOk) {
            ...
            HL7v25.populateERR(reply.getERR(), "QPD", "1", "3", "1", "4", "204", "域ID不是有效范围内的域ID");
            return reply;
        }
        ...
        if ( !validPatient ) {
            ...
            HL7v25.populateERR(reply.getERR(), "QPD", "1", "3", "1", "1", "204", "病人ID为： " + ConversionHelper.getPersonIdentifier(requestPatientId).getIdentifier() + " 的记录不存在");
