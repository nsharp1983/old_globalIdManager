...
import com.ats.aexchange.audit.jaxb.ActiveParticipantType;
import com.ats.aexchange.audit.jaxb.AuditMessage;
import com.ats.aexchange.audit.jaxb.AuditSourceIdentificationType;
import com.ats.aexchange.audit.jaxb.CodedValueType;
import com.ats.aexchange.audit.jaxb.EventIdentificationType;
import com.ats.aexchange.audit.jaxb.ObjectFactory;
import com.ats.aexchange.audit.jaxb.ParticipantObjectIdentificationType;
import com.ats.aexchange.audit.jaxb.TypeValuePairType;
...
public class AuditObjectFactory {
	...
	public AuditObjectFactory(IMessageTransmitter messenger) {
		...
		activeParticipants = new ArrayList<AuditMessage.ActiveParticipant>();
		...
		throws JAXBException
	{
		...
		for (AuditMessage.ActiveParticipant apt: activeParticipants) list.add(apt);
		...
		throws JAXBException
	{
		...
		JAXBContext messageContext = JAXBContext.newInstance("com.ats.aexchange.audit.jaxb");
	...
	protected void addActiveParticipant( ActiveParticipant activeParticipant ) 
		throws JAXBException 
	{
		AuditMessage.ActiveParticipant participant = messageFactory.createAuditMessageActiveParticipant();
		// Required:
		...
		throws JAXBException 
	{
		...
		if (participantObject.idTypeCode.getCodeSystemName() != null) {
			participantID.setCodeSystemName(participantObject.idTypeCode.getCodeSystemName());
		}
		if (participantObject.idTypeCode.getName() != null) {
			participantID.setDisplayName(participantObject.idTypeCode.getName());
		}
		
		for(TypeValuePair typeValue : participantObject.getDetails()) {
			TypeValuePairType tvpt = messageFactory.createTypeValuePairType();
			tvpt.setType(typeValue.getType());
			tvpt.setValue(typeValue.getValue().getBytes());
